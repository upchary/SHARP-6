...
public interface UserRepositoryy extends JpaRepository<UserMaster, String> {
	...
	@Query(value = "SELECT * FROM UserMaster WHERE email = ?1 and password = ?2 and activestatus=1", nativeQuery = true)
	UserMaster getUser(String emailAddress, String password);

	@Query(value = "SELECT * FROM LoginSession WHERE email = ?1 and password = ?2", nativeQuery = true)
	LoginSession userLogin(String emailAddress, String password);

	@Query(value = "update logouttime=?2 FROM LoginSession WHERE userId = ?1 ", nativeQuery = true)
	LoginSession userLogout(String userId, String logouttime);

	@Query(value = "update password=?2 FROM LoginSession WHERE userId = ?1 ", nativeQuery = true)
